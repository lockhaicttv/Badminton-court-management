...
const Banner = () => {
    ...
    const handleEditBanner = () => {
        ...
        callApi(`court/edit-image/banner/${courtInfo._id}`, 'put', banner)
            .then((res) => {
    ...
    const handleEditLogo = () => {
        callApi(`court/edit-image/logo/${courtInfo._id}`, 'put', courtInfo.logo)
            .then((res) => {
    ...
    const modalEditBanner = <Modal show={isShowModal} onHide={handleClose}>
    ...
    const modalEditLogo = <Modal show={showEditLogo} onHide={handleCloseEditLogo}>
        <Modal.Header closeButton>
        ...
        <div className='banner pb-5'>
            ...
            <div className='logo rounded-circle'>
                <div className='position-relative'>
                    ...
                    <button className='edit-logo rounded btn'  width='24px' height='48px'>
                        <img src='/image/camera-icon.png'
                             onClick={handleCloseEditLogo}/>
                    </button>
                </div>
            </div>
            <Button variant='outline-dark' className='edit-banner rounded'
                    onClick={handleClose}
            >
                Chỉnh sửa hình banner
            </Button>
            {modalEditBanner}
            {modalEditLogo}
        </div>
