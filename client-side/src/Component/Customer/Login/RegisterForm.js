...
const RegisterForm = (props) => {
  ...
  const handleCheckUserName = (e) => {
    if (e.target.value !== "") {
      callApi(`account/check-exist/${user.username}`, "get", null).then(
        (res) => {
          if (res.data.message.includes("tồn tại")) {
            setCheckUsername(res.data.message);
          } else {
            callApi(`user/check-exist/${user.username}`, "get", null).then(
              (res) => {
  ...
  return (
    <Row>
      <Col>
        ...
        <p>
          Tạo tài khoản để theo dõi đơn hàng, lưu danh sách sản phẩm, hoá đơn,
          chứng từ.
        </p>
      </Col>
      <Col>{userForm}</Col>
